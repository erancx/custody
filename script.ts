function isWithKids(inputDate: Date): boolean {
    ...
    const daysDifference = Math.floor((inputDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24));
...
function checkCustody(): void {
    const dateInput = (<HTMLInputElement>document.getElementById('dateInput')).value;
